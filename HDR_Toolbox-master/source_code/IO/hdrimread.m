...
switch extension
    ...
    % ===================== 【线性 PNG HDR 专用读取】 =====================
%16-bit PNG used as relative HDR data
    case 'png'
        try
            raw = imread(filename);
            img = double(raw);   % 保留原始线性强度，不除以 65535
            bLDR = 0;

            hdr_info = struct();
            hdr_info.loaded = 1;
            hdr_info.format = 'png_linear_hdr';
        catch
            hdr_info = struct('loaded', 0);
            warning('This .png file cannot be read as linear HDR data.');
        end
end
